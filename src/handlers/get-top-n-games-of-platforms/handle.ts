...
import type { GetTopNGamesOfPlatformsReply } from '../../../proto-gen/coresrv_pb';
import { GetTopNGamesOfPlatformsRequest } from '../../../proto-gen/coresrv_pb';
...
export function createHandler(
  ...
  async function handle(
    ...
    const grpcRequest = new GetTopNGamesOfPlatformsRequest()
      .setN(query.n);

    await new Promise<[
      ServiceError | null,
      GetTopNGamesOfPlatformsReply,
    ]>(
      resolve => {
        const meta = new Metadata({ idempotentRequest: true });
        meta.set('auth', authToken);
        deps.coreService.getTopNGamesOfPlatforms(
          grpcRequest,
          meta,
      ...
      .then(async ([error, response]) => {
        ...
        return await reply
          ...
          .send(
            Object.fromEntries(
              responseObject.groupMap.map(([k, v]) => [k, v.itemsList]),
            ),
          );
      });
  }
