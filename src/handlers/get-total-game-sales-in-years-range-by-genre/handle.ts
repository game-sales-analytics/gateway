...
import type { GetTotalGameSalesInYearsRangeByGenreReply } from '../../../proto-gen/coresrv_pb';
import { GetTotalGameSalesInYearsRangeByGenreRequest } from '../../../proto-gen/coresrv_pb';
...
export function createHandler(
  ...
  async function handle(
    ...
    const grpcRequest = new GetTotalGameSalesInYearsRangeByGenreRequest()
      .setStartYear(query.startYear)
      .setEndYear(query.endYear);

    await new Promise<[
      ServiceError | null,
      GetTotalGameSalesInYearsRangeByGenreReply,
    ]>(
      resolve => {
        const meta = new Metadata({ idempotentRequest: true });
        meta.set('auth', authToken);
        deps.coreService.getTotalGameSalesInYearsRangeByGenre(
          grpcRequest,
          meta,
      ...
      .then(async ([error, response]) => {
        ...
        return await reply
          ...
          .send(
            responseObject.genreTotalSalesMap
              .map(([k, v]) => ({ genre: k, totalSales: v })),
          );
      });
  }
