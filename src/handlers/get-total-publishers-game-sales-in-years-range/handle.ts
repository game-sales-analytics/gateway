...
import type { GetTotalPublishersGameSalesInYearsRangeReply } from '../../../proto-gen/coresrv_pb';
import { GetTotalPublishersGameSalesInYearsRangeRequest } from '../../../proto-gen/coresrv_pb';
...
function parseRequestQuery(query: unknown): RequestQuery {
  ...
  return {
    publisherIds: m.get('publisherIds') as string[],
...
export function createHandler(
  ...
  async function handle(
    ...
    deps.logger.debug({ query: request.query });
    ...
    const grpcRequest = new GetTotalPublishersGameSalesInYearsRangeRequest()
      .setStartYear(query.startYear)
      .setEndYear(query.endYear)
      .setPublisherIdsList(query.publisherIds);

    await new Promise<[
      ServiceError | null,
      GetTotalPublishersGameSalesInYearsRangeReply,
    ]>(
      resolve => {
        const meta = new Metadata({ idempotentRequest: true });
        meta.set('auth', authToken);
        deps.coreService.getTotalPublishersGameSalesInYearsRange(
          grpcRequest,
          meta,
      ...
      .then(async ([error, response]) => {
        ...
        return await reply
          ...
          .send(
            Object.fromEntries(
              responseObject.publisherSalesMap
                .map(([k, v]) => [k, v.itemsList]),
            ),
          );
      });
  }
