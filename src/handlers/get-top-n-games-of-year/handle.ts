...
import type { GetTopNGamesOfYearReply } from '../../../proto-gen/coresrv_pb';
import { GetTopNGamesOfYearRequest } from '../../../proto-gen/coresrv_pb';
...
export function createHandler(
  ...
  async function handle(
    ...
    const grpcRequest = new GetTopNGamesOfYearRequest()
      .setYear(query.year)
      .setN(query.n);

    await new Promise<[ServiceError | null, GetTopNGamesOfYearReply]>(
      resolve => {
        const meta = new Metadata({ idempotentRequest: true });
        meta.set('auth', authToken);
        deps.coreService.getTopNGamesOfYear(
          grpcRequest,
          meta,
