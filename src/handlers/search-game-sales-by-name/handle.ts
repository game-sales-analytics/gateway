...
import type { SearchGameSalesByNameReply } from '../../../proto-gen/coresrv_pb';
import { SearchGameSalesByNameRequest } from '../../../proto-gen/coresrv_pb';
...
export function createHandler(
  ...
  async function handle(
    ...
    const grpcRequest = new SearchGameSalesByNameRequest()
      .setName(query.name);

    await new Promise<[ServiceError | null, SearchGameSalesByNameReply]>(
      resolve => {
        const meta = new Metadata({ idempotentRequest: true });
        meta.set('auth', authToken);
        deps.coreService.searchGameSalesByName(
          grpcRequest,
          meta,
