...
import type { Get5MostSoldGamesByYearAndPlatformReply } from '../../../proto-gen/coresrv_pb';
import { Get5MostSoldGamesByYearAndPlatformRequest } from '../../../proto-gen/coresrv_pb';
...
function parseRequestQuery(query: unknown): RequestQuery {
  ...
  return {
    platform: m.get('platform') as string,
    year: m.get('year') as number,
  };
}
...
export function createHandler(
  ...
  async function handle(
    ...
    const grpcRequest = new Get5MostSoldGamesByYearAndPlatformRequest()
      .setPlatform(query.platform)
      .setYear(query.year);

    await new Promise<[
      ServiceError | null,
      Get5MostSoldGamesByYearAndPlatformReply,
    ]>(
      resolve => {
        const meta = new Metadata({ idempotentRequest: true });
        meta.set('auth', authToken);
        deps.coreService.get5MostSoldGamesByYearAndPlatform(
          grpcRequest,
          meta,
